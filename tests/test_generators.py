...
from model_tools.generators import PairedEmbeddingGenerator, SequentialGenerator, RandomBlockGenerator


def get_dummy_data(row_count, row_width=27):
    """this function returns dummy images, label data in pd.DataFrame"""

    images = pd.DataFrame([ [ j * i for j in range(row_width)] for i in range(row_count)])
...
def no_load_peg(self,batch_size=64,data_rows=128, row_width=27):
    if batch_size:
